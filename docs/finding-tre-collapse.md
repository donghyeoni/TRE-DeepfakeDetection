# TRE ≈ 0 수렴 근거 정리

> tre-diffusion-image-detection 재현 실험 중 확인된 핵심 발견.
> 나중에 리포 `docs/`에 포함하거나 README Notes로 요약 예정.

## 주장

원본 구현(노트북 및 리포 `src/data/`)의 TRE(Temporal Reconstruction Error)
특징은 **이론적으로 정확히 0**이며, 실제 저장되는 값은 GPU 부동소수점
비결정성 잔차다. 따라서 분류기는 의도된 "시간적 재구성 오차"가 아니라
수치 잔차의 패턴을 학습한다.

## 근거 1 — 수학적: edit-friendly 인버전의 정확 복원 성질

구현이 사용하는 인버전은 edit-friendly DDPM inversion
(Huberman-Spiegelglas et al.)이다. 순방향에서 노이즈 latent 열
`x_1..x_T`를 샘플링한 뒤(`sample_xts_from_x0`), 각 스텝의 노이즈를

```
z_t = (x_{t-1} - mu_t(x_t)) / sigma_t        # inversion_forward_process
```

로 **역산해 저장**한다. 역과정(`inversion_reverse_process` +
`reverse_step`)은 같은 `mu_t`, `sigma_t` 공식으로

```
x_{t-1} = mu_t(x_t) + sigma_t * z_t
```

를 계산하므로, 저장된 `z_t`를 그대로 재주입하면 **구성상 x_{t-1}이 정확히
재현**된다. 귀납적으로, 어떤 스텝 k의 `x_k`에서 출발하든 재구성 결과는
정확히 `x_0`(원본 latent)이다.

TRE는 "prefix k로 복원한 latent"와 "prefix k+1로 복원한 latent"의 차이인데
(`over_denosing`, `LoadDataset.ipynb` cell 6 = 리포 `tre_features.py`),
위 성질에 의해 **모든 prefix 복원이 동일한 x_0**이므로 차이는 0이다.

(유일한 예외: 구현이 `zs[0] = 0`으로 마지막 스텝 노이즈를 지우는데, 이
편차는 모든 prefix 복원에 공통으로 들어가므로 차이에서 상쇄된다.)

## 근거 2 — 실측: 서버 재현에서의 통계

GPU 서버(L40S, fp32, torch 2.3.1+cu121, diffusers 0.31.0)에서 원본 코드
경로(`src.data.tre_features.compute_tre`)로 실측:

```
TRE shape: (20, 4, 32, 32), dtype float32
mean ≈ 4.5e-14,  std ≈ 2.7e-4
```

SD latent의 자연 스케일이 O(1)임을 감안하면 신호 크기가 약 3~4 자릿수
작다. 0이 아닌 이유는 UNet 추론의 커널 비결정성(같은 입력이라도 실행마다
미세하게 다른 부동소수점 결과) 때문이며, 이는 순방향(z 역산 시)과
역방향(재주입 시)의 UNet 출력이 비트 단위로 일치하지 않아 생긴다.

## 근거 3 — 원본 노트북과의 대조

- `LoadDataset.ipynb` cell 6의 `over_denosing`은 리포 코드와 동일하게
  `inversion_reverse_process(..., zs=zs[:step+1])`로 **z_t를 재주입**한다.
  즉 이 성질은 리팩터링으로 생긴 것이 아니라 원본부터 존재했다.
- 같은 노트북의 분류기 학습 셀(cell 16)은 `temporal_module`이 정의되지
  않아 실행 불가능한 상태로 남아 있다 — 실험이 완주되지 않았음을 시사.

## 성능 수치에 대한 주의

원논문 보고 수치(Ours_TRE 66.0% 등)는 실험이 제대로 완료되지 않은
상태에서 나온 것으로 **신뢰할 수 없다**(저자 확인). 따라서 본 재현에서는
논문 수치를 재현 목표나 비교 기준으로 삼지 않으며, 이번 전체 규모
재실행(학습 60k / 평가 96k, 프로토콜 문서화)이 **이 파이프라인의 첫 유효
측정치**가 된다.

## 재현 실측 결과 (전체 규모)

학습 sdv1.4 30k+30k(seed 42) / 평가 8개 생성기 전량, 20 epochs.
분류기·프로토콜은 원본 노트북 레시피 그대로. 원본 수치는 `results/repro.json`.

| Generator | Accuracy | AP |
| --- | --- | --- |
| sdv4 (학습 도메인) | 61.5% | 0.642 |
| sdv5 | 62.2% | 0.648 |
| wukong | 59.8% | 0.623 |
| glide | 58.1% | 0.593 |
| vqdm | 56.2% | 0.562 |
| midjourney | 55.0% | 0.552 |
| biggan | 54.7% | 0.560 |
| adm | 54.4% | 0.541 |
| **평균** | **57.7%** | — |

해석: **학습 도메인에서조차 61.5%**에 머물고, 보지 못한 생성기에서는
54~60%(우연 수준 50%에 근접)로 떨어진다. 즉 z 재주입 TRE에는 실질적인
판별 신호가 거의 없고, 남은 약한 신호는 본 문서가 분석한 수치 잔차의
이미지 의존적 패턴으로 설명된다. 학습 과정도 이와 일관되게 val acc가
52~61%에서 진동했다.

(참고: 초기 sanity 실험에서 val acc 79.5%가 관측됐으나, 당시 데이터가
fake 79% 불균형이어서 다수 클래스 예측에 수렴한 착시였다 — val acc가
fake 비율과 일치.)

## 개선 실험 설계 (fresh-noise TRE)

재주입 대신 **미리 뽑은 공통 난수 열 eps_t를 모든 prefix 복원에 공유**하여
재구성한다:

- prefix별 복원이 실제로 달라져 TRE가 "모델이 이미지를 얼마나 안정적으로
  설명하는가"를 측정하게 됨.
- 난수를 prefix 간 공유(common random numbers)하여 순수 난수 분산이
  아니라 prefix 길이 효과만 차이에 남도록 함.
- `zs[0]=0` 관행은 동일하게 유지(eps[0]=0)해 프로토콜 차이를 최소화.
- 그 외 모든 조건(T=20, 256px, SD v1.4, 학습 60k/평가 96k, 분류기,
  하이퍼파라미터) 동일 — 비교 변인은 노이즈 재주입 여부 하나.

구현: `extract_tre.py --fresh` (서버), 결과는 `features_fresh/` +
`results_fresh.json`으로 저장하여 재현 결과(`results.json`)와 비교.

## 개선 실험 결과 — 실패 (음성 결과)

동일 규모·동일 분류기로 재실행. 원본 수치는 `results/fresh.json`.

| 조건 | 학습 acc | 홀드아웃 val acc | 8생성기 평균 acc |
| --- | --- | --- | --- |
| 재현 (z 재주입) | 78.7% | 63.5% (best) | **57.7%** |
| fresh-noise | 96.3% | 50.7% (best) | **50.1%** |

생성기별로도 fresh 조건은 전부 49.3~50.7% — **모든 생성기에서 정확히 우연
수준**이다. 학습 정확도만 96%까지 올라간다.

원인 분석: eta=1 확률적 역과정에서 각 스텝의 분산항 `sigma_t * eps_t`는
스케일이 크고, prefix가 길수록 서로 다른 개수의 분산항이 누적된다. 따라서
prefix 간 차이(TRE)는 "모델이 이미지를 얼마나 잘 설명하는가"보다 **그 이미지에
할당된 난수 열 자체**에 지배된다. 난수는 이미지마다 새로 뽑히므로 분류기는
학습 샘플의 난수 패턴을 암기할 수 있을 뿐(train 96%) 어떤 것도 전이되지
않는다(val·test 50%). prefix 간 난수를 공유(common random numbers)한 것은
prefix 길이가 다르면 누적되는 항의 개수 자체가 달라진다는 점을 해소하지
못했다.

결론: **확률적(eta=1) 재구성 위에서는 prefix 차이 방식으로 유효한 신호를
얻을 수 없다.** z 재주입은 차이를 0으로 붕괴시키고, 난수 주입은 차이를
난수로 덮는다. 유효한 신호를 얻으려면 확률항 자체를 제거해야 하며, 이는
`docs/ideas-generalization.md`의 eta=0 결정론적 재구성 오차(DIRE/LaRE 계열)
방향이 다음 단계임을 실험적으로 지지한다.

## 조건 C (eta=0) 결과 — 신호는 살아났고, 일반화에서 막혔다

확률항을 제거하고(DDIM 인버전 + 분산항 없는 역과정) 동일 규모·동일 분류기로
재실행. 원본 수치는 `results/eta0.json`. 추출은 L40S 8장(서버 2대)으로
분담했고, 두 서버의 리스트 순서 지문이 동일함을 검증했다.

| 조건 | 학습 acc | 홀드아웃 val | 8생성기 평균 |
| --- | --- | --- | --- |
| A. z 재주입 | 78.7% | 63.5% | 57.7% |
| B. fresh 난수 | 96.3% | 50.7% | 50.1% |
| **C. eta=0** | — | **80.9%** | **60.5%** |

생성기별로 보면 평균 60.5%가 실제 양상을 감춘다 — **모델 계열에 따라 정확히
갈린다**:

| 생성기 | acc | AP | 계열 |
| --- | --- | --- | --- |
| sdv4 (학습 도메인) | 78.5% | .863 | SD 계열 |
| sdv5 | 79.2% | .869 | SD 계열 |
| wukong | 78.5% | .861 | SD 계열(중국어 SD) |
| midjourney | 54.8% | .560 | 기타 |
| vqdm | 53.3% | .542 | 기타 |
| adm | 50.6% | .510 | 기타 |
| glide | 49.1% | .481 | 기타 |
| biggan | **39.8%** | .360 | GAN |

- **SD 계열 평균 78.8%** (AP 0.86) — 처음으로 실질적인 탐지기가 됐다.
- **비SD 계열 평균 49.5%** — 우연 수준. biggan은 39.8%로 **우연보다 낮다**,
  즉 학습한 방향이 반대로 작동한다(GAN latent는 SD 인버터가 자기 샘플처럼
  왕복시키지 못한다).

해석: eta=0 TRE는 "SD v1.4가 이 이미지를 얼마나 잘 왕복시키는가"를 재는
지표다. 따라서 인버터와 **같은 계열** 생성기에는 강력하지만, 다른 확산 구조나
GAN에는 전이되지 않는다. DIRE/LaRE 계열이 공통으로 지적받는 한계와 같은 벽이다.

세 조건의 실패 양상이 서로 다르다는 점이 이 연구의 결론이다:
**A는 신호가 없고, B는 신호가 난수에 묻히고, C는 신호가 한 생성기 계열에만
특화된다.** 단일 인버터의 재구성 오차만으로 일반화된 생성이미지 탐지에
도달하려는 접근 자체가 축을 잘못 잡았을 가능성이 크다.

## 계열 편향의 원인 규명 (후속 실험 3종)

C의 계열 편향이 어디서 오는지를 세 실험으로 좁혔다. 원본 수치는
`results/{logo,threeclass,ensemble}.json`.

### (1) 학습 편향인가, 특징의 성질인가 — leave-one-generator-out

5개 생성기로 학습해 6번째를 평가하는 방식을 6회 반복.

| 홀드아웃 | adm | biggan | glide | midjourney | vqdm | wukong | 평균 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 홀드아웃 acc | 52.3 | 53.0 | 53.0 | 53.4 | 48.0 | 50.4 | **51.7** |
| 학습된 5개 val | 53.1 | 54.0 | 51.8 | 53.4 | 58.2 | 52.8 | — |

- 미지 생성기 전이는 **전무**(전부 우연 수준).
- **학습에 쓴 생성기들에서도 52~58%**밖에 안 된다. 생성기를 섞으면 도메인 내
  적합조차 무너진다.
- 결정적으로 wukong(SD 계열)이 sdv1.4 학습 때 78.5%였다가 비SD 5개로 학습하면
  50.4%로 떨어진다. **SD 신호는 SD 계열 학습 데이터에서만 배울 수 있다.**

→ 계열 편향은 학습 구성의 문제가 아니라 **특징 자체의 성질**이다. eta=0 TRE는
"SD1.4 인버터의 왕복 정확도"를 인코딩하며, 다른 생성기를 아무리 학습에 넣어도
그 자리를 대신하지 못한다.

### (2) biggan 역전은 구조인가 잡음인가 — 3클래스 헤드

real / diffusion-fake / GAN-fake로 헤드를 넓히자 **biggan 역전이 사라졌다**
(39.8% → 63.4%, AP 0.36 → 0.68). 나머지 비SD 생성기도 학습에 포함되면
57~61%로 올라온다. 순수 홀드아웃 이진 정확도는 54.3%로 절대 수치는 여전히
낮지만, **역전이 잡음이 아니라 클래스 구조였다**는 점이 확인됐다.

### (3) 두 번째 인버터가 계열 결속을 끊는가 — 앙상블

SD1.4와 SD1.5의 eta=0 특징을 채널축으로 결합해 8채널 입력으로 학습.

| 그룹 | C 단독 | C + 2번째 인버터 |
| --- | --- | --- |
| SD 계열 (sdv4/sdv5/wukong) | 78.8% | **78.9%** |
| 그 외 5개 | 49.5% | **52.7%** |

SD 계열은 변화가 없고, 비SD는 3%p 남짓 올랐을 뿐이다(biggan 39.8→45.5%).
SD1.5 단독 통제 조건도 SD1.4 단독과 동일한 수치를 내(sdv4 78.7 vs 78.5) 두
인버터가 **같은 것을 보고 있음**을 확인했다.

단, 이 실험은 **자기 질문에 답하지 못한다**. 의도했던 두 번째 인버터는
SD 2.1이었으나 `stabilityai/*` 저장소가 전부 게이트되어(익명 접근 401) 접근할
수 없었고, 대체재인 SD 1.5는 1.4의 연속 학습 모델이라 **같은 계열**이다.
따라서 이 결과는 "앙상블이 무용하다"가 아니라 **"같은 계열끼리의 앙상블은
무용하다"**로 읽어야 한다. 진짜 검증에는 픽셀공간 ADM이나 GAN 인버전처럼
계열이 다른 인버터가 필요하다.

## 베이스라인 정책

STRE 베이스라인은 직접 재실험하지 않고 **원문(Ren et al.) 공표 수치를
인용**한다 (필요 시 저자가 직접 재실험 예정). 인용 시 프로토콜 차이(데이터
서브셋 구성, 해상도, 학습 규모)가 있을 수 있음을 비교표에 명시할 것.

## 재현 기록 (실험 환경)

- 데이터: GenImage (HF 미러 jzousz/GenImage), 학습 SDv1.4 train 30k+30k
  (seed 42), 평가 8개 생성기 test 12.5k씩 (genimage_test.zip)
- 특징 추출: 원본 O(T²) prefix 복원 의미 보존, 이미지 배치(48)만 병렬화,
  fp32 연산 / fp16 저장
- 분류기: AttentionClassifier(MHSA embed4/head4/2층 + SpatialFocusing +
  ResNet18-4ch), CE loss, Adam 1e-4, batch 16 — LoadDataset.ipynb 레시피,
  head 수만 embed 제약(4의 약수)으로 조정
- 수정한 원본 잠재 버그: (1) SpatialFocusing 5D 입력 불일치 → 채널 평균
  전달, (2) build_dataset.py의 ImageFolder ai/nature 라벨 반전(추출기는
  명시 라벨 사용으로 회피)
