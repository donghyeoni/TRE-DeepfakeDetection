...
실측 경과: 초기 sanity(불균형 데이터, fake 79%)에서 val acc 79.5%가
나왔으나 이는 다수 클래스 예측에 수렴한 착시로 확인됨 (val acc ≈ fake
비율). 균형 잡힌 60k 본학습에서는 val acc가 52~61%에서 진동 — "z 재주입
TRE는 분류 신호가 거의 없다"는 본 문서의 진단을 실험적으로 뒷받침한다.
최종 값은 results.json(도메인 내·교차 생성기)으로 확정한다.
