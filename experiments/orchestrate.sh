...
while [ $(grep -l ALL_DONE extract_gpu*.log 2>/dev/null | wc -l) -lt $NSHARDS ]; do sleep 300; done
...
CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=2 ./venv/bin/python train_eval.py --features "$TRE_HOME/features_fresh" --epochs 20 --batch 16 --ckpt weights/ours_tre_fresh.pt --results results_fresh.json > train_fresh.log 2>&1
log "ALL_PHASES_DONE"
