...
log "start shard $K"
./venv/bin/python extract_tre.py --list data/lists/train.txt --out features/train --batch 48 --shard $K --nshards $NSHARDS
log "train shard done"
for gen in sdv4 sdv5 adm biggan glide midjourney vqdm wukong; do
  log "test $gen"
  ./venv/bin/python extract_tre.py --list data/lists/test_$gen.txt --out features/test/$gen --batch 48 --shard $K --nshards $NSHARDS
done
